  ...
  loop
    ...
    loop
      ...
      if v_collection = 'lessonSections' then
        ...
        set payload = jsonb_set(payload, v_path[2:array_length(v_path, 1)], v_value, true),
            updated_at = now()
        where id = v_id and source = 'seed';
      elsif v_collection = 'lessonTexts' then
        if v_path = array['content'] then
          update public.hanzihome_lesson_texts
          set content = v_value #>> '{}', updated_at = now()
          where id = v_id and source = 'seed';
        elsif v_path = array['title'] then
          update public.hanzihome_lesson_texts
          set title = v_value #>> '{}', updated_at = now()
          where id = v_id and source = 'seed';
        else
