...
import {
 ...
 type DailyReadingEnrichmentModule,
} from "@/features/daily-reading/daily-reading.schemas";
import type {
 DailyReadingEnrichmentArticle,
 DailyReadingEnrichmentResponse,
 DailyReadingGeneratedBy,
} from "@/features/daily-reading/daily-reading-enrichment.schemas";
import {
 requestDailyReadingEnrichmentProvider,
 type DailyReadingProviderMetrics,
 type DailyReadingProviderModule,
} from "@/features/daily-reading/daily-reading-enrichment-provider.server";
...
type ProviderFailure = Extract<DailyReadingEnrichmentResponse, { ok: false }>;
...
function targetLevelLabel(reading: DailyReadingEnrichmentArticle) {
 return (
  reading.classification.targetLevel ?? reading.classification.estimatedLevel ?? "unspecified"
 );
}

function articleEvidence(reading: DailyReadingEnrichmentArticle) {
...
async function requestStructured<T, Validated>(input: {
 ...
 module: DailyReadingProviderModule;
 prompt: string;
 schema: z.ZodType<T>;
 signal?: AbortSignal;
 validate(data: T): Validated;
 maximumAttempts?: number;
 onMetrics?: (metrics: DailyReadingProviderMetrics) => void;
 ...
 for (let attempt = 0; attempt < maximumAttempts; attempt += 1) {
  const response = await requestDailyReadingEnrichmentProvider({
  ...
  input.onMetrics?.(response.metrics);
  if (!response.ok) {
   return {
    ok: false,
...
function translationUnits(reading: DailyReadingEnrichmentArticle): TranslationUnit[] {
...
export function createDailyReadingTranslationPlan(reading: DailyReadingEnrichmentArticle) {
...
export function generateDailyReadingTranslationMetadata(input: {
 reading: DailyReadingEnrichmentArticle;
 runtime: ResolvedUserAiRuntime;
 signal?: AbortSignal;
 onMetrics?: (metrics: DailyReadingProviderMetrics) => void;
...
export function generateDailyReadingTranslationGroup(input: {
 reading: DailyReadingEnrichmentArticle;
 runtime: ResolvedUserAiRuntime;
 units: readonly TranslationUnit[];
 chunkLabel: string;
...
export function buildDailyReadingTranslationResponse(input: {
 reading: DailyReadingEnrichmentArticle;
 runtime: ResolvedUserAiRuntime;
 ...
 const data = dailyReadingTranslationDataSchema.parse({
...
async function generateTranslation(
 ...
 onMetrics?: (metrics: DailyReadingProviderMetrics) => void,
): Promise<DailyReadingEnrichmentResponse> {
 const translatedParagraphs: TranslationDraft["paragraphs"] = [];
 const plan = createDailyReadingTranslationPlan(reading);
 const metadata = await generateDailyReadingTranslationMetadata({
  reading,
  runtime,
  signal,
  onMetrics,
 ...
 return buildDailyReadingTranslationResponse({
  reading,
  runtime,
...
function validateVocabulary(
 ...
 return dailyReadingVocabularyDataSchema.parse({
...
function articleSentences(reading: DailyReadingEnrichmentArticle) {
...
function validateGrammar(
 ...
 return dailyReadingGrammarDataSchema.parse({
...
function validateQuestions(
 ...
 return dailyReadingQuestionsDataSchema.parse({
...
function learningPrompt(
 reading: DailyReadingEnrichmentArticle,
 module: Exclude<DailyReadingEnrichmentModule, "translation">,
 targetCount: number,
) {
 const common = [
...
function generatedBy(runtime: ResolvedUserAiRuntime): DailyReadingGeneratedBy {
 return {
...
export function generateDailyReadingEnrichment(input: {
 reading: DailyReadingEnrichmentArticle;
 runtime: ResolvedUserAiRuntime;
