...
import type { DailyReadingCaptureResponse } from "@/features/daily-reading/daily-reading.schemas";
...
import { previewDailyReadingSource } from "@/features/daily-reading/daily-reading.client";

const values = new Map<string, string>();

const responsePayload: DailyReadingCaptureResponse = {
 reading: {
  schemaVersion: "2.0.0",
...
describe("Daily Reading source preview", () => {
 beforeEach(() => {
  values.clear();
 ...
 it("uses the current capture contract without persisting a preview article or capture run", async () => {
  ...
  expect(values.has("chines-app:daily-reading")).toBe(false);
