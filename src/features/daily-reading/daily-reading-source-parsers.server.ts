...
function pageTitle(html: string) {
 ...
 return textFromFragment(metadataTitle || heading)
  .replace(/\s+/gu, " ")
  .trim();
}
...
function jsonLdArticleBodies(html: string) {
 return Array.from(
  ...
  return Array.from(script.matchAll(/["']articleBody["']\s*:\s*"((?:\\.|[^"\\])*)"/giu)).flatMap(
   (bodyMatch) => {
...
function collectParagraphs(fragment: string, plainText: boolean) {
 ...
 const rawParagraphs = plainText
  ? cleaned.split(/\n{1,}/gu)
  : Array.from(cleaned.matchAll(/<p\b[^>]*>([\s\S]*?)<\/p>/giu)).map((match) => match[1] ?? "");
