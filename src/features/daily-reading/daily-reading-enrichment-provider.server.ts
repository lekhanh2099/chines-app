...
export type DailyReadingProviderModule = "translation" | "vocabulary" | "grammar" | "questions";

export type DailyReadingProviderMetrics = {
 latencyMs: number;
...
function outputLimit(module: DailyReadingProviderModule) {
 switch (module) {
  case "translation":
   return 6_000;
  case "questions":
...
function providerFailure(input: {
 ...
 errorName?: string;
 metrics: DailyReadingProviderMetrics;
}): Extract<DailyReadingProviderResult, { ok: false }> {
  ...
  try {
   ...
   if (!response.ok) {
    ...
    return providerFailure({
     ...
     metrics: providerMetrics(latencyMs, null, null),
    });
   }
   ...
   const failure = caughtFailure(runtime, error, providerMetrics(latencyMs, null, null), signal);
   if (
...
export function requestDailyReadingEnrichmentProvider(input: {
 runtime: ResolvedUserAiRuntime;
 prompt: string;
 module: DailyReadingProviderModule;
 schema: z.ZodType;
