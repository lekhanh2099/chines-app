...
import type { LearningProgressItem } from "@/features/hanzihome/types";
import {
 getReviewDueAt,
 isReviewDue,
...
describe("review scheduler", () => {
 it("does not put untouched items into the due queue", () => {
  ...
  expect(isReviewDue({ level: 0, status: "new" }, new Date("2026-08-14T00:00:00.000Z"))).toBe(false);
 });
 ...
 it("derives the due date from the persisted level and last review time", () => {
  const progress: LearningProgressItem = {
   level: 3,
   status: "known",
 ...
 it("advances a known answer by one stage", () => {
  const progress = nextScheduledProgress(undefined, "known", new Date("2026-08-14T00:00:00.000Z"));
  expect(progress).toEqual({ level: 1, status: "known", lastReviewedAt: "2026-08-14T00:00:00.000Z" });
 ...
 it("resets again answers to a short relearning interval", () => {
  const progress = nextScheduledProgress(
   { level: 4, status: "known", lastReviewedAt: "2026-08-01T00:00:00.000Z" },
   "again",
 ...
 it("keeps hard answers at their current stage and reviews them the next day", () => {
  const progress = nextScheduledProgress(
   { level: 3, status: "known", lastReviewedAt: "2026-08-01T00:00:00.000Z" },
   "hard",
   new Date("2026-08-14T00:00:00.000Z"),
  );
