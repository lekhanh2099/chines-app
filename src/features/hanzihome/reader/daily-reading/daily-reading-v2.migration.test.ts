...
import { dailyReadingRunSchema, dailyReadingSchema } from "./daily-reading.schemas";
import {
 createDailyReadingV2Fingerprint,
 migrateDailyReadingV1ItemToV2,
 migrateDailyReadingV1LedgerToV2,
} from "./daily-reading-v2.migration";

const legacyReading = dailyReadingSchema.parse({
 schemaVersion: "1.0.0",
 ...
 grammarPoints: Array.from({ length: 3 }, (_value, index) => ({
  ...
  evidenceSentenceZh: "对很多年轻人来说，博物馆不只是保存文物的地方，也是理解城市和历史的一种入口。",
 })),
 questions: [
  {
   id: "q1",
...
const legacyRun = dailyReadingRunSchema.parse({
 id: "run-legacy-1",
 date: "2026-08-19",
 kind: "manual",
 status: "succeeded",
...
describe("Daily Reading V2 migration", () => {
 it("preserves the readable legacy article while marking its provenance honestly", () => {
  const migrated = migrateDailyReadingV1ItemToV2(legacyReading);
 ...
 it("creates a deterministic fingerprint from source identity and Chinese content", () => {
  const first = createDailyReadingV2Fingerprint({
  ...
  const second = createDailyReadingV2Fingerprint({
   sourceUrl: legacyReading.source.url,
   titleZh: legacyReading.titleZh,
   paragraphsZh: legacyReading.paragraphs.map((paragraph) => paragraph.zh),
  });
  const changed = createDailyReadingV2Fingerprint({
   sourceUrl: legacyReading.source.url,
 ...
 it("retains V1 run history separately instead of mislabeling it as V2 capture history", () => {
  const ledger = migrateDailyReadingV1LedgerToV2({ items: [legacyReading], runs: [legacyRun] });
