...
import type { DailyReadingEnrichmentModule } from "./daily-reading.schemas";
...
import { getDailyReadingSnapshot, saveDailyReadingArticle } from "./daily-reading-storage.client";
...
const readyTranslation: Extract<DailyReading["enrichment"]["translation"], { status: "ready" }> = {
 status: "ready",
...
function blockedJob(id: string, module: DailyReadingEnrichmentModule, taskId: AiTaskId) {
 return {
  id,
  runId,
  workflowRunId: null,
...
describe("Daily Reading enrichment client", () => {
 beforeEach(() => {
  values.clear();
 ...
 it("marks all pending learning modules blocked after missing-key without touching the article", async () => {
  ...
  const updated = await enrichDailyReadingLearningSupport(article.id);
 ...
 it("persists a successful module without resetting another module or sending sibling state", async () => {
  const before = getDailyReadingSnapshot().items.find((item) => item.id === article.id);
  ...
  const updated = await enrichDailyReadingModule(article.id, "translation");
  const requestBody = String(fetchMock.mock.calls[0]?.[1]?.body ?? "");
  ...
  expect(getDailyReadingSnapshot().enrichmentRuns[0]).toMatchObject({
 ...
 it("keeps an uncertain enqueue pending without binding work to page lifecycle", async () => {
  ...
  await expect(enrichDailyReadingModule(article.id, "translation")).rejects.toMatchObject({
   code: "network-error",
  });
 ...
 it("hydrates a completed server job into the matching local article", async () => {
  ...
  const queued = await enrichDailyReadingModule(article.id, "translation");
  expect(queued.enrichment.translation.status).toBe("running");

  await reconcilePendingDailyReadingEnrichmentJobs();

  const hydrated = getDailyReadingSnapshot().items.find((item) => item.id === article.id);
