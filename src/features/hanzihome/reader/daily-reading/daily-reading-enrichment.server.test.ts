...
vi.mock("./daily-reading-enrichment-provider.server", () => ({
 requestDailyReadingEnrichmentProvider: mocks.requestProvider,
}));

import { generateDailyReadingEnrichment } from "./daily-reading-enrichment.server";
...
describe("Daily Reading enrichment generation", () => {
 beforeEach(() => {
 ...
 it("chunks long translations without dropping source paragraphs", async () => {
  const longReading: DailyReading = {
   ...reading,
   article: {
    ...reading.article,
 ...
 it("rejects grammar examples that are not complete sentences from the source", async () => {
  const invalidGrammar = {
   items: [
    {
     patternZh: "利用……",
     explanationVi: "Giải thích 1",
     evidenceSentenceZh: "一个不在原文里的句子。",
    },
    ...
    items: [
     {
      type: "main_idea",
      promptZh: "主旨是什么？",
      promptVi: "Ý chính là gì?",
      answerZh: "介绍博物馆的新展览。",
      answerVi: "Giới thiệu triển lãm mới.",
      evidenceParagraphIds: ["missing-p"],
     },
     {
      type: "detail",
      promptZh: "谁来看展览？",
      promptVi: "Ai xem triển lãm?",
      answerZh: "年轻观众。",
      answerVi: "Khán giả trẻ.",
      evidenceParagraphIds: ["source-p1"],
     },
     {
      type: "detail",
      promptZh: "团队用了什么？",
      promptVi: "Nhóm dùng gì?",
      answerZh: "器物照片和互动资料。",
      answerVi: "Ảnh hiện vật và tư liệu tương tác.",
      evidenceParagraphIds: ["source-p2"],
     },
     {
      type: "inference",
      promptZh: "为什么合作？",
      promptVi: "Vì sao hợp tác?",
      answerZh: "为了持续学习。",
      answerVi: "Để duy trì việc học.",
      evidenceParagraphIds: ["source-p3"],
     },
     {
      type: "summary",
      promptZh: "请概括全文。",
      promptVi: "Hãy tóm tắt.",
