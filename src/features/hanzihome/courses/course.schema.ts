...
export const hanziHomeCourseTypeSchema = z.string().trim().min(1).default("custom");
...
export const createHanziHomeCourseRequestSchema = z.object({
 ...
 initialBookTitle: z.string().trim().min(1, "Vui lòng nhập tên quyển/sách đầu tiên"),
 initialBookShortTitle: z.string().trim().optional(),
});

export type CreateHanziHomeCourseRequest = z.infer<typeof createHanziHomeCourseRequestSchema>;
