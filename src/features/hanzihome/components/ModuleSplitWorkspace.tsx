...
import {
 HanziHomeStudyTabs,
 type HanziHomeStudyTab,
} from "@/features/hanzihome/components/HanziHomeStudyTabs";
...
import { HanziHomeEditingTools, useDraftPatchedLesson } from "@/features/hanzihome/editing";
...
function parseStudyModule(value: string | null | undefined): StudyModule | null {
...
function readLessonViewMode(): LessonViewMode {
 ...
 return window.localStorage.getItem(lessonViewModeKey) === "debug" ? "debug" : "study";
}
...
function setPaneActive(layout: PaneLayout, paneId: PaneId, module: StudyModule) {
...
export function ModuleSplitWorkspace({
 ...
 const moveModule = (module: StudyModule, targetPane: PaneId, targetIndex: number) => {
 ...
 const renderModule = (module: StudyModule, compact = false) => {
  switch (module) {
   ...
   case "lessonText":
    return <LessonTextInlineEditor lesson={patchedLesson} compact={compact} />;
   case "notes":
 ...
 const debugPanel =
  developerToolsEnabled && activeViewMode === "debug" && activeModule !== "overview" ? (
