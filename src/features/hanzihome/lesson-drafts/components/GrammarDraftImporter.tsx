...
function mergeNotes(
  ...
  return {
    overviewMarkdown:
      [currentNotes.overviewMarkdown, incomingNotes.overviewMarkdown]
        .filter(Boolean)
        .join("\n\n---\n\n"),
    grammarSummary:
      [currentNotes.grammarSummary, incomingNotes.grammarSummary]
        .filter(Boolean)
        .join("\n"),
    vocabularyText:
      [currentNotes.vocabularyText, incomingNotes.vocabularyText]
        .filter(Boolean)
        .join("\n"),
    properNounsText:
      [currentNotes.properNounsText, incomingNotes.properNounsText]
        .filter(Boolean)
        .join("\n"),
    applicationMarkdown:
      [currentNotes.applicationMarkdown, incomingNotes.applicationMarkdown]
        .filter(Boolean)
        .join("\n\n---\n\n"),
    ...
    <div className="grid gap-4">
      ...
      <Field label="Ứng dụng / điển tích / đọc thêm">
        <Textarea
          ...
          onChange={(event) => updateNotes({ applicationMarkdown: event.target.value })}
          className="min-h-36"
        />
      </Field>
...
export function GrammarDraftImporter({ draft }: GrammarDraftImporterProps) {
  ...
  const saveGrammar = async (
    ...
    const parsedItems = nextItems.map((item) => grammarDraftItemSchema.parse(item));
  ...
  const handleImportAndSave = async (options: { strict: boolean }) => {
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Không lưu được ngữ pháp");
    }
  };
          ...
          <Card padding="lg" className="rounded-2xl">
            <div className="grid gap-4">
              ...
              <div className="grid gap-3 text-sm font-semibold text-text-muted">
                <p>
                  <b>Ghi chú bài</b>: tổng quan, từ vựng liên quan, tên riêng, phần ứng dụng.
                </p>
                <p>
                  <b>Điểm ngữ pháp</b>: title, pattern, công thức, ví dụ, bẫy sai, so sánh.
                </p>
                <p>
