...
import { getCurrentCalibrationItem } from "@/features/hanzihome/personal-learning/application/personal-learning.calibration";
import {
...
export function PersonalLearningHubPage({ mode }: { mode: HubMode }) {
 ...
 const pendingHypotheses = store.hypotheses.filter(
  (item) =>
   item.status === "pending" || item.status === "needs-intent" || item.status === "uncertain",
 );
    ...
    {mode === "dashboard" ? (
     <>
      <section
       ...
       aria-label="Tóm tắt học cá nhân"
      >
       ...
       <Card variant="section" padding="lg" className="grid gap-3">
        <div className="grid gap-1">
         ...
         <Typography tone="secondary">
          Hiệu chuẩn trước nếu chưa có đủ bằng chứng; sau đó app mới tăng độ tin cậy của trạng thái.
         </Typography>
        </div>
        <div className="flex flex-wrap gap-2">
         <Button asChild size="toolbar">
          <Link href="/personal-learning/calibration">
       ...
       <Card variant="section" padding="lg" className="grid gap-3">
        ...
        {store.evidence.length === 0 ? (
         <Typography tone="muted">
          Chưa có bằng chứng. Bắt đầu bằng calibration hoặc luyện tập.
         </Typography>
        ) : (
         store.evidence
          .slice(-4)
          ...
          .map((item) => (
           <div key={item.id} className="flex flex-wrap items-center justify-between gap-2">
            <Typography weight="bold">
             {getKnowledgeNode(item.knowledgeNodeId).shortLabelVi} · {item.dimension}
            </Typography>
            <Badge
             variant={
      ...
      {reviewNodes.map((node) => {
       ...
       return (
        ...
        >
         ...
         <Button asChild size="toolbar" variant="outline">
          <Link href={`/personal-learning/knowledge/${node.id}`}>
           Ôn node này
           <ArrowRight data-icon="inline-end" />
          </Link>
         </Button>
      ...
      {personalLearningKnowledgeNodes.map((node) => {
       ...
       return (
        <Card key={node.id} variant="section" padding="lg" className="grid gap-3">
         ...
         <Button asChild variant="link" size="inline" align="start">
          <Link href={`/personal-learning/evidence/${node.id}`}>Xem bằng chứng</Link>
         </Button>
        </Card>
       );
      })}
     ...
     <section className="grid gap-3 xl:grid-cols-2">
      {personalLearningKnowledgeNodes.map((node) => (
       <Card key={node.id} variant="section" padding="lg" className="grid gap-3">
        ...
        <Typography tone="secondary">{node.coreQuestionVi}</Typography>
        <Button asChild size="toolbar" variant="outline" align="start">
         <Link href={`/personal-learning/knowledge/${node.id}`}>
          Mở kiến thức
          <ArrowRight data-icon="inline-end" />
         </Link>
     ...
     <section className="grid gap-3">
      {store.hypotheses.length === 0 ? (
       <Card variant="subtle" padding="lg">
        <Typography tone="muted">Chưa có giả thuyết lỗi nào.</Typography>
       </Card>
      ) : (
          ...
          <div className="grid gap-1">
           <div className="flex flex-wrap items-center gap-2">
            <Typography weight="black">
             {getKnowledgeNode(hypothesis.knowledgeNodeId).shortLabelVi}
            </Typography>
            <Badge variant="default">{hypothesis.status}</Badge>
           </div>
           <Typography tone="secondary">
            {attempt?.originalInput ?? hypothesis.proposedSubtype}
           </Typography>
           <Typography variant="caption" tone="muted">
            {hypothesis.explanationVi}
           </Typography>
          </div>
          <Button asChild size="toolbar" variant="outline">
           <Link href={`/personal-learning/errors/${hypothesis.id}`}>
            Review
            <ArrowRight data-icon="inline-end" />
           </Link>
          </Button>
...
function MetricCard({
 ...
 return (
  <Card variant="section" padding="lg" className="grid gap-2">
   ...
   <Typography variant="pageTitle" weight="black">
    {value}
   </Typography>
  </Card>
 );
...
export function PersonalLearningKnowledgeDetailPage({ nodeId }: { nodeId: string }) {
 ...
 const state =
  store.nodeStates.find((item) => item.knowledgeNodeId === node.id)?.state ?? "UNOBSERVED";
 return (
  <PageContainer>
   <main className="grid w-full min-w-0 gap-5">
    <PageHeader
     eyebrow="Personal Learning · Knowledge"
     title={node.titleVi}
     description={node.coreQuestionVi}
     meta={
      <div className="flex flex-wrap gap-2">
       <StateBadge state={state} />
       <Badge variant="info">{node.reviewStatus}</Badge>
    ...
    <Card variant="section" padding="lg" className="grid gap-3">
     ...
     <ol className="grid gap-2 pl-5 list-decimal">
      {node.decisionStepsVi.map((step) => (
       <li key={step}>
        <Typography tone="secondary">{step}</Typography>
       </li>
      ))}
     </ol>
     <div className="flex flex-wrap gap-2">
      {node.frames.map((frame) => (
       <Badge key={frame} variant="default">
        {frame}
       </Badge>
      ))}
     </div>
    ...
    <Card variant="section" padding="lg" className="grid gap-3">
     ...
     <ul className="grid gap-2 pl-5 list-disc">
      {node.markedCasesVi.map((item) => (
       <li key={item}>
        <Typography tone="secondary">{item}</Typography>
       </li>
      ))}
     </ul>
    </Card>
    <Card variant="section" padding="lg" className="grid gap-3">
     <div className="flex flex-wrap items-center justify-between gap-2">
      <Typography as="h2" variant="sectionTitle" weight="black">
       Nguồn & khoảng trống nghiên cứu
      </Typography>
      <Badge variant="warning">Không tự nâng review status</Badge>
     </div>
     {node.sources.map((source) => (
      <div key={source.id} className="grid gap-1">
       <Typography weight="bold">{source.title}</Typography>
       <Typography variant="caption" tone="muted">
        {source.locator} · {source.noteVi}
       </Typography>
      </div>
     ))}
     <ul className="grid gap-2 pl-5 list-disc">
      {node.researchGapsVi.map((gap) => (
       <li key={gap}>
        <Typography tone="secondary">{gap}</Typography>
       </li>
      ))}
     </ul>
    </Card>
...
export function PersonalLearningEvidencePage({ nodeId }: { nodeId: string }) {
 ...
 return (
  <PageContainer>
   <main className="grid w-full min-w-0 gap-5">
    <PageHeader
     eyebrow="Personal Learning · Evidence"
     title={`Bằng chứng · ${node.shortLabelVi}`}
     description="Lịch sử bằng chứng M1–M5 dùng để suy trạng thái; dữ liệu này không phải điểm phần trăm tự gán."
    />
    <PersonalLearningNav />
    <section className="grid gap-3">
     {evidence.length === 0 ? (
      <Card variant="subtle" padding="lg">
       <Typography tone="muted">Chưa có bằng chứng cho node này.</Typography>
      </Card>
     ) : (
      evidence.map((item) => (
       <Card key={item.id} variant="section" padding="lg" className="grid gap-2">
        <div className="flex flex-wrap items-center gap-2">
         <Badge variant="info">{item.dimension}</Badge>
         <Badge
          variant={
           item.outcome === "success"
            ? "success"
         ...
         <Badge variant="default">{item.opportunityType}</Badge>
        </div>
        <Typography variant="caption" tone="muted">
         {item.mode} · hint {item.hintLevel} · {item.observedAt}
        </Typography>
...
export function PersonalLearningErrorDetailPage({ hypothesisId }: { hypothesisId: string }) {
 ...
 if (hypothesis === undefined)
  return <PersonalLearningNotFound title="Không tìm thấy giả thuyết lỗi" />;
 const attempt = store.attempts.find((item) => item.id === hypothesis.attemptId);
 if (attempt === undefined)
  return <PersonalLearningNotFound title="Không tìm thấy lần thử nguồn" />;
 const latestIntent =
  store.intentRevisions
   .filter((item) => item.attemptId === attempt.id)
   .sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0]?.intendedMeaningVi ??
  attempt.intendedMeaningVi ??
  "";
 return (
  <ErrorReviewForm
   key={`${hypothesis.id}:${latestIntent}`}
   learning={learning}
   hypothesisId={hypothesis.id}
   originalInput={attempt.originalInput}
   initialIntent={latestIntent}
   explanation={hypothesis.explanationVi}
   intentQuestion={hypothesis.intentQuestionVi}
   status={hypothesis.status}
  />
 );
}

function ErrorReviewForm({
 learning,
 hypothesisId,
 originalInput,
 initialIntent,
 ...
 const [grammaticality, setGrammaticality] = useState<ErrorAnnotation["grammaticality"] | null>(
  null,
 );
 const [meaningAccuracy, setMeaningAccuracy] = useState<ErrorAnnotation["meaningAccuracy"] | null>(
  null,
 );
 ...
 const accept = () => {
  ...
  if (intent.trim() !== initialIntent.trim())
   learning.addPersonalLearningIntent(
    learning.state.progress.personalLearning?.hypotheses.find((item) => item.id === hypothesisId)
     ?.attemptId ?? "",
    intent.trim(),
   );
  learning.resolvePersonalLearningHypothesis(hypothesisId, "accepted", {
   grammaticality,
   meaningAccuracy,
   naturalness,
   registerFit,
 ...
 return (
  <PageContainer>
   <main className="grid w-full min-w-0 gap-5">
    <PageHeader
     eyebrow="Personal Learning · Error review"
     title="Xác nhận trước khi học từ lỗi"
     description="Detector chỉ tạo giả thuyết. Chỉ khi ý định người học và bốn chiều chất lượng đã rõ thì hypothesis mới được chuyển thành annotation."
     meta={<Badge variant="default">{status}</Badge>}
    />
    <PersonalLearningNav />
    <Card variant="section" padding="lg" className="grid gap-3">
     <Typography as="h2" variant="sectionTitle" weight="black">
     ...
     {intentQuestion ? (
      <Typography tone="info" weight="bold">
       {intentQuestion}
      </Typography>
     ) : null}
    </Card>
    <Card variant="section" padding="lg" className="grid gap-4">
     <label className="grid gap-2">
      <Typography variant="label" weight="bold">
       Ý bạn muốn nói bằng tiếng Việt
      </Typography>
      <Input
       value={intent}
     ...
     <QualityChoice
      ...
      values={["valid", "invalid", "marked", "uncertain"]}
      onChange={setGrammaticality}
      disabled={resolved}
     />
     <QualityChoice
      label="Độ đúng nghĩa"
      value={meaningAccuracy}
      values={["preserved", "partially-preserved", "changed", "unknown"]}
      onChange={setMeaningAccuracy}
      disabled={resolved}
     />
     <QualityChoice
      label="Độ tự nhiên"
      value={naturalness}
      values={["natural", "acceptable", "awkward", "unknown"]}
      onChange={setNaturalness}
      disabled={resolved}
     />
     <QualityChoice
      label="Độ hợp văn phong"
      value={registerFit}
      values={["fits", "too-formal", "too-casual", "genre-bound", "unknown"]}
      onChange={setRegisterFit}
      disabled={resolved}
     />
     <div className="flex flex-wrap gap-2">
      ...
      <Button
       variant="outline"
       disabled={resolved}
       onClick={() => learning.resolvePersonalLearningHypothesis(hypothesisId, "uncertain")}
      >
       Chưa đủ chắc
      </Button>
      <Button
       variant="secondary"
       disabled={resolved}
       onClick={() => learning.resolvePersonalLearningHypothesis(hypothesisId, "rejected")}
      >
       Bác giả thuyết
      </Button>
     </div>
...
function QualityChoice<T extends string>({
 label,
 value,
 values,
 onChange,
 ...
 return (
  <fieldset className="grid gap-2">
   <Typography as="legend" variant="label" weight="bold">
    {label}
   </Typography>
   <div className="flex flex-wrap gap-2">
    {values.map((item) => (
     <Button
      key={item}
      size="toolbar"
      variant={value === item ? "active" : "outline"}
  ...
  <PageContainer>
   <main className="grid w-full min-w-0 gap-5">
    <PageHeader
     ...
     title="Hiệu chuẩn"
     description="Mỗi câu tạo attempt và mastery evidence thật. Trả lời sai hoặc thiếu chắc chắn sẽ chèn thêm câu parallel thay vì gắn nhãn yếu ngay lập tức."
    />
    <PersonalLearningNav />
    {session === null || session.status === "completed" ? (
     <Card variant="section" padding="lg" className="grid gap-3">
      <Typography as="h2" variant="sectionTitle" weight="black">
       {session?.status === "completed" ? "Đợt hiệu chuẩn đã hoàn tất" : "Chưa có đợt hiệu chuẩn"}
      </Typography>
      <Typography tone="secondary">
       App sẽ bỏ qua các node đang AUTOMATING hoặc STABLE_TRANSFER; nếu tất cả đã ổn định thì dùng
       lại bộ primary để kiểm tra chuyển giao.
      </Typography>
      <Button onClick={learning.startOrResumePersonalLearningCalibration} size="toolbar">
       <Play data-icon="inline-start" />
       Bắt đầu
      </Button>
     </Card>
     ...
     <CalibrationItemForm
      key={`${session.id}:${currentItem.id}`}
      item={currentItem}
      index={session.currentIndex}
      total={session.itemIds.length}
      onSubmit={learning.submitPersonalLearningCalibrationAnswer}
      onPause={learning.pausePersonalLearningCalibration}
     />
    ) : (
     <Card variant="subtle" padding="lg">
      <Typography tone="muted">Không tìm thấy câu calibration hiện tại.</Typography>
     </Card>
    )}
   </main>
  </PageContainer>
...
function CalibrationItemForm({
 ...
 item: NonNullable<ReturnType<typeof getCurrentCalibrationItem>>;
 index: number;
 total: number;
 onSubmit: ReturnType<typeof useLearningState>["submitPersonalLearningCalibrationAnswer"];
 onPause: () => void;
}) {
 ...
 return (
  <Card variant="section" padding="lg" className="grid gap-5">
   <div className="flex flex-wrap items-center justify-between gap-2">
    <Badge variant="info">
     {item.knowledgeNodeId} · {item.dimension}
    </Badge>
    <Typography variant="caption" tone="muted">
     {index + 1}/{total}
    </Typography>
   ...
   <div className="grid gap-2">
    ...
    <div className="flex flex-wrap gap-2">
     {item.options.map((option) => (
      <Button
       key={option.id}
       variant={selectedOptionId === option.id ? "active" : "outline"}
    ...
    <div className="flex flex-wrap gap-2">
     {(["sure", "unsure", "guess"] as const).map((itemConfidence) => (
      <Button
       key={itemConfidence}
       size="toolbar"
       variant={confidence === itemConfidence ? "active" : "outline"}
       aria-pressed={confidence === itemConfidence}
       onClick={() => setConfidence(itemConfidence)}
      >
       {itemConfidence}
      </Button>
     ))}
   ...
   <div className="grid gap-2">
    <Typography variant="label" weight="bold">
     Tự sửa / giải thích thêm (không bắt buộc)
    </Typography>
    <Textarea
     value={selfCorrectionText}
     onChange={(event) => setSelfCorrectionText(event.target.value)}
     placeholder={item.selfCorrectionPromptVi}
    />
   </div>
  ...
  <PageContainer>
   <main className="grid gap-5">
    <PageHeader
     eyebrow="Personal Learning"
     title={title}
     description="Đường dẫn không khớp dữ liệu hiện có."
    />
    <Button asChild variant="outline">
     <Link href="/personal-learning">Về tổng quan</Link>
    </Button>
   </main>
  </PageContainer>
 );
