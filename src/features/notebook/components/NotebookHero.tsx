 ...
 return (
  <section className="nova-gradient-hero overflow-hidden rounded-3xl bg-[linear-gradient(125deg,#12bdb1_0%,#478bff_50%,#8358f7_100%)] p-5 text-white sm:p-7 lg:grid lg:grid-cols-[minmax(17rem,0.72fr)_minmax(0,2fr)] lg:gap-7">
   <div>
    ...
    <p className="mt-4 max-w-lg text-sm font-semibold leading-6 text-white/90">
     Nhận diện dấu hiệu, vị trí trong câu, sắc thái và ví dụ đối chiếu thay vì học theo một bản dịch
     cố định.
    </p>
   </div>

   <div className="mt-6 self-center lg:mt-0">
    <div className="grid grid-cols-3 gap-2">
     {metrics.map((metric) => {
      ...
      return (
       <div
        key={metric.label}
        className="rounded-2xl border border-white/65 bg-white/70 px-3 py-4 text-center text-[#20233a] shadow-[inset_0_1px_0_rgba(255,255,255,.9),0_12px_30px_rgba(38,32,104,.13)] backdrop-blur-2xl"
       >
    ...
    <p className="mt-3 rounded-2xl border border-white/25 bg-white/10 px-4 py-3 text-sm font-semibold leading-6 text-white/90 backdrop-blur">
