     ...
     <Card key={comparison.id} variant="glass" padding="lg">
      ...
      <div className="flex items-start gap-3">
       ...
       <div className="grid gap-1">
        ...
        <p className="text-sm font-medium text-text-muted">{comparison.note}</p>
       </div>
      </div>
      ...
      {comparedItems.length > 0 ? (
       <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {comparedItems.map((item) => (
         <div key={item.id} className="rounded-2xl border border-border-default bg-bg-card/80 p-4 grid gap-3">
          ...
          <p className="font-bold text-text-secondary">{item.vi}</p>
          <p className="text-sm font-medium leading-6 text-text-muted">{item.essence}</p>
          <p className="rounded-xl bg-accent-subtle px-3 py-2 font-mono text-sm font-bold text-accent-text">
           {item.pattern}
          </p>
         </div>
        ))}
      ...
      <div className="grid gap-3 lg:grid-cols-2">
       <div className="rounded-xl border border-success/20 bg-success-subtle p-4 grid gap-2">
        ...
        <p className="text-sm font-semibold leading-6 text-text-primary">{comparison.rule}</p>
       </div>
