...
import { humanitiesDocumentResponseSchema } from "@/features/humanities/model/humanities-resource.schemas";
import {
 readerAssetRowSchema,
 type ReaderAssetRow,
 type ReaderPdfAsset,
} from "@/features/reading/model/reading-assets.schemas";
import {
...
export async function getReaderDocument(documentId: string) {
 ...
 const resource = {
  ...
  exerciseItems: itemsResult,
  assets,
 };
 return document.kind === "humanities"
  ? humanitiesDocumentResponseSchema.parse(resource)
  : readerDocumentResponseSchema.parse(resource);
}
