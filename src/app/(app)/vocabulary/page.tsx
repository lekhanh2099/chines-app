...
import { useVocabEntries, VocabSchemaMissingError } from "@/features/vocabulary/hooks/useVocabEntries";
...
function getMeaning(entry: VocabEntryWithProgress) {
 return entry.ai_analysis.meaning_detail || entry.ai_analysis.definitions?.[0]?.meaning || entry.meaning;
}
...
function getStudyCharacters(entry: VocabEntryWithProgress) {
 return Array.from(entry.hanzi).filter((character) => /[\u3400-\u9fff]/.test(character));
}

function entryHasMissingData(entry: VocabEntryWithProgress) {
 const analysis = entry.ai_analysis;
 return !analysis.decomposition || !analysis.examples?.length || !analysis.collocations?.length || !analysis.usage_note;
}
...
function examplesToText(examples?: AiExample[]) {
 return (examples || [])
  .map((example) => [example.zh, example.pinyin, example.vi, example.note || ""].join(" | "))
  .join("\n");
}
...
function textToExamples(value: string): AiExample[] {
 return value
  ...
  .filter(Boolean)
  .map((line) => {
   const [zh = "", pinyin = "", vi = "", note = ""] = line.split("|").map((part) => part.trim());
...
function getMarkdownSection(block: string, sectionNumber: number) {
 const pattern = new RegExp(`\\*\\*${sectionNumber}\\.\\s*[^*]+\\*\\*([\\s\\S]*?)(?=\\n\\*\\*\\d+\\.|\\n---|$)`, "i");
...
function parseMarkdownExamples(section: string): AiExample[] {
 ...
 return chunks
  .map((chunk) => {
   ...
   const first = lines[0] || "";
   const sentenceMatch = first.match(/^\d+\.\s*(.+?)\(([^)]+)\)\s*$/) || first.match(/^(.+?)\(([^)]+)\)\s*$/);
   const zh = sentenceMatch?.[1]?.replace(/^\d+\.\s*/, "").trim() || first.replace(/^\d+\.\s*/, "");
   const pinyin = sentenceMatch?.[2]?.trim() || "";
   const vi = lines.find((line) => !line.startsWith("→") && line !== first) || "";
   const note = lines.find((line) => line.startsWith("→"))?.replace(/^→\s*/, "");
   return { zh, pinyin, vi, note };
  })
  .filter((example) => example.zh || example.vi);
}

function parseMarkdownVocabEntries(value: string, lesson: VocabLessonWithStats): ImportedEntryDraft[] {
 const blocks = value
  .split(/\n---+\n|(?=\n##\s+)/g)
 ...
 return blocks.map((block, index) => {
  const header = block.match(/^##\s*(.+?)\s+[–-]\s+(.+?)\s+[–-]\s+(.+?)\s+[–-]\s+(.+)$/m);
...
function applyProgress(
 ...
 const lessons = course.lessons.map((lesson) => {
  ...
  return {
   ...
   learning,
   fresh,
   progress: entries.length ? Math.round((mastered / entries.length) * 100) : 0,
  };
 });
...
function isCoursePayload(data: VocabCourseWithLessons | { course: null; lessons: []; entries: [] } | undefined): data is VocabCourseWithLessons {
...
export default function VocabularyPage() {
 ...
 const [editingEntry, setEditingEntry] = useState<VocabEntryWithProgress | null>(null);
 const [editingLesson, setEditingLesson] = useState<VocabLessonWithStats | null>(null);
 const [importingLesson, setImportingLesson] = useState<VocabLessonWithStats | null>(null);
 ...
 const activeLessonEntries = useMemo(() => activeLesson?.entries || [], [activeLesson?.entries]);
 ...
 const visibleEntries = useMemo(() => {
  const base = (activeTab === "all" ? course?.entries || [] : activeLessonEntries).filter((entry) => matchesEntry(entry, searchQuery));
  return base.filter((entry) => {
   if (wordFilter === "all") return true;
   if (wordFilter === "examples") return Boolean(entry.ai_analysis.examples?.length);
 ...
 const quizOptions = useMemo(() => {
  ...
  const sameCategory = activeLessonEntries.filter(
   (entry) => entry.id !== activeEntry.id && entry.category && entry.category === activeEntry.category,
  );
  const fallback = (course?.entries || []).filter((entry) => entry.id !== activeEntry.id);
  const unique = [...sameCategory, ...activeLessonEntries, ...fallback]
   ...
   .sort((a, b) => hashForStudyOrder(`${a.id}:${activeEntry.id}`) - hashForStudyOrder(`${b.id}:${activeEntry.id}`))
   .slice(0, 3);
  return [activeEntry, ...unique].sort(
   (a, b) => hashForStudyOrder(`${a.id}:${activeEntry.id}:option`) - hashForStudyOrder(`${b.id}:${activeEntry.id}:option`),
  );
 ...
 const updateProgress = useCallback(
  async (entry: VocabEntryWithProgress, nextLevel: number) => {
   ...
   queryClient.setQueryData(["vocab-entries"], applyProgress(course, entry.id, nextLevel));
 ...
 const goNextCard = useCallback(() => {
  resetCardState();
  setCardIndex((index) => (studyQueue.length ? (index + 1) % studyQueue.length : 0));
 ...
 const rememberEntry = useCallback(
  async (entry = activeEntry) => {
   ...
   setSessionStats((stats) => ({ ...stats, seen: stats.seen + 1, remembered: stats.remembered + 1 }));
   goNextCard();
 ...
 const missEntry = useCallback(
  async (kind: "again" | "review", entry = activeEntry) => {
   ...
   setSessionStats((stats) => ({ ...stats, seen: stats.seen + 1, missed: stats.missed + 1 }));
   goNextCard();
   if (kind === "again") await updateProgress(entry, Math.max((entry.proficiency_level || 0) - 1, 0));
  },
  [activeEntry, goNextCard, updateProgress],
 );
 ...
 const saveEntry = useCallback(
  async (entry: VocabEntryWithProgress, analysis: AiAnalysis) => {
   ...
   const response = await fetch(isNew ? "/api/vocab/entries" : `/api/vocab/entries/${entry.id}`, {
 ...
 const deleteEntry = useCallback(
  async (entry: VocabEntryWithProgress) => {
   ...
   const response = await fetch(`/api/vocab/entries/${entry.id}`, { method: "DELETE" });
   const result = (await response.json().catch(() => null)) as { error?: string } | null;
   if (!response.ok) {
 ...
 const saveLesson = useCallback(
  async (lesson: VocabLessonWithStats) => {
   ...
   const response = await fetch(isNew ? "/api/vocab/lessons" : `/api/vocab/lessons/${lesson.id}`, {
    method: isNew ? "POST" : "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
 ...
 const deleteLesson = useCallback(
  async (lesson: VocabLessonWithStats) => {
   ...
   if (!window.confirm(`Xoá "${lesson.title}" và ${lesson.entries.length} từ trong bài này?`)) return;
   const response = await fetch(`/api/vocab/lessons/${lesson.id}`, { method: "DELETE" });
   const result = (await response.json().catch(() => null)) as { error?: string } | null;
   if (!response.ok) {
 ...
 const createLessonDraft = useCallback(() => {
  if (!course) return;
  const maxOrder = lessons.reduce((max, lesson) => Math.max(max, lesson.lesson_order), 0);
 ...
 const resetImport = useCallback(async () => {
  setResetting(true);
  try {
   const response = await fetch("/api/vocab/import/docx-reset", { method: "POST" });
   const result = (await response.json().catch(() => null)) as { entryCount?: number; error?: string } | null;
 ...
 useEffect(() => {
  const handleKeyDown = (event: KeyboardEvent) => {
   ...
   if (event.key === " ") {
    ...
    else if ((mode === "quiz" || mode === "reverse") && answerChecked) continueAfterQuiz();
 ...
 if (error) {
  ...
  return (
   <LearningShell>
    <EmptyState
     title={missingSchema ? "Chưa apply database migration" : "Không tải được từ vựng"}
     description={
      missingSchema
       ...
       : error instanceof Error ? error.message : "Kiểm tra lại API hoặc migration."
     }
    />
   </LearningShell>
  );
  ...
  <LearningShell>
   ...
   {isLoading ? (
    <div className="flex min-h-[520px] items-center justify-center rounded-[28px] border-2 border-stone-200 bg-white shadow-theme-md">
     ...
     <span className="ml-3 text-sm font-black text-stone-500">Đang tải kho từ...</span>
    </div>
   ) : !course ? (
    <EmptyState
     title="Chưa có course từ vựng"
     description="Import file Vocabulary Compilation.docx để bắt đầu học theo bài."
     action={<ActionButton onClick={resetImport} loading={resetting} icon={Upload}>Import docs</ActionButton>}
    />
   ) : (
    <>
  ...
  <header className="rounded-[28px] border-2 border-stone-200 bg-white p-4 shadow-theme-sm md:p-5">
   <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
    <div>
     ...
     <h1 className="mt-4 text-4xl font-black tracking-normal text-stone-900">Từ vựng Hán ngữ</h1>
     <p className="mt-2 max-w-xl text-base font-bold leading-7 text-stone-500">
      Học theo bài, ôn bằng flashcard, trắc nghiệm và chỉnh dữ liệu ngay khi cần.
     </p>
    </div>
   ...
   {activeTab === "study" && (
    <div className="mt-5 flex flex-wrap items-center gap-2">
     <LearningSegmentedControl
      value={mode}
      items={studyModes.map((item) => ({ key: item.key, label: item.label, icon: item.icon }))}
     ...
     <IconToolButton icon={Settings} label="Cài đặt" />
     <IconToolButton icon={HelpCircle} label="Shortcut" onClick={onShowShortcuts} />
    </div>
   )}
  </header>
 );
...
function ActionButton({
 ...
 return (
  <button
   ...
   className={cn("inline-flex h-11 items-center gap-2 rounded-2xl border-2 px-4 text-sm font-black shadow-theme-sm transition disabled:opacity-60", className)}
  >
...
function LessonCard({
 ...
 return (
  ...
  >
   <span className={cn("flex h-11 w-11 shrink-0 items-center justify-center rounded-full border-2 text-sm font-black", active ? "border-white/60 bg-white/20" : "border-stone-200 bg-white")}>
    {lesson.progress}%
   </span>
   <span className="min-w-0 flex-1">
    <span className="flex items-center justify-between gap-2">
     <span className="truncate text-xl font-black">{lesson.title.replace(/^Tổng quan & Phân nhóm từ vựng\s*/i, "")}</span>
     {onEdit && (
      <span
       role="button"
       tabIndex={0}
       ...
       className={cn("rounded-xl p-2 opacity-0 transition group-hover:opacity-100", active ? "hover:bg-white/20" : "hover:bg-stone-100")}
      >
       <PenLine className="h-4 w-4" />
      </span>
     )}
    </span>
    <span className={cn("mt-1 block text-sm font-bold", active ? "text-white/90" : "text-stone-500")}>
     {lesson.mastered}/{lesson.entries.length} thẻ
    </span>
    <span className="mt-2 flex flex-wrap gap-1">
     {lesson.categories.slice(0, 3).map((category) => (
      <span
       key={category.name}
       className={cn("rounded-full px-2 py-0.5 text-[10px] font-black", active ? "bg-white/20 text-white" : "bg-stone-100 text-stone-500")}
      >
 ...
 return (
  <section className="rounded-[28px] border-2 border-stone-200 bg-white p-5 shadow-theme-md md:p-7">
   <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
    <div>
     <p className="text-sm font-black uppercase tracking-wide text-red-500">{lesson.title}</p>
     <h2 className="mt-1 text-3xl font-black text-stone-900">{lesson.title.replace(/^Tổng quan & Phân nhóm từ vựng\s*/i, "")}</h2>
     <p className="mt-2 text-base font-bold text-stone-500">
      {lesson.entries.length} từ trong nhóm này · {lesson.categories.slice(0, 2).map((item) => item.name).join(", ")}
     </p>
    </div>
   ...
   <StudyCard>
    {!activeEntry ? (
     <EmptyState title="Không có từ trong bài" description="Chọn bài khác hoặc import lại docs." compact />
    ) : props.mode === "guess" ? (
   ...
   <div className="mt-5 grid gap-3 md:grid-cols-3">
    <ActionButton onClick={props.onAgain} tone="neutral">Chưa nhớ</ActionButton>
    <ActionButton onClick={props.onReview} tone="neutral">Cần ôn lại</ActionButton>
    <ActionButton onClick={props.onRemember}>Đã nhớ</ActionButton>
   </div>
  </section>
 );
}
 ...
 return (
  <section className="mt-7">
   <div className="flex flex-wrap items-end justify-between gap-3">
    <div>
     <p className="text-lg font-black uppercase tracking-wide text-stone-600">Tiến độ - {title}</p>
     <p className="mt-1 text-sm font-bold text-stone-500">
      Đã học {stats.seen} · Nhớ {stats.remembered} · Cần ôn {stats.missed}
      {randomMode ? " · Đang xáo bài" : ""}
     </p>
    </div>
    <p className="text-xl font-black uppercase tracking-wide text-stone-700">{current} / {total} thẻ</p>
   </div>
...
function StudyMeta({ entry, cardIndex, total }: { entry: VocabEntryWithProgress; cardIndex: number; total: number }) {
 return (
  <div className="flex flex-wrap items-center justify-center gap-2">
   <span className="text-sm font-black uppercase tracking-wide text-red-500">{entry.source.lessonKey}</span>
   {entry.category && <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-stone-500 shadow-theme-sm">{entry.category}</span>}
   <span className="text-sm font-black text-stone-400">Thẻ {Math.min(cardIndex + 1, total)} / {total}</span>
  </div>
 );
}
 ...
 return (
  <div className="mx-auto flex w-full max-w-4xl flex-col items-center">
   ...
   <div className="relative mt-6 w-full max-w-3xl">
    ...
    <div className="absolute inset-0 rotate-[3deg] rounded-[36px] border-2 border-stone-100 bg-white/80" />
    <div className="relative flex min-h-[520px] flex-col rounded-[36px] border-2 border-stone-100 bg-white p-7 shadow-theme-md">
     <div className="flex items-center justify-between text-stone-800">
      <button type="button" className="rounded-full p-2 hover:bg-stone-50" title="Yêu thích">
       <Heart className="h-7 w-7" />
      </button>
      <span className="text-lg font-black text-stone-500">{Math.min(cardIndex + 1, total)} / {total}</span>
      <button type="button" onClick={onSpeak} className="rounded-full p-2 hover:bg-stone-50" title="Phát âm">
       <Volume2 className="h-7 w-7" />
      </button>
     </div>
     {!revealed ? (
      <button type="button" onClick={onReveal} className="flex flex-1 flex-col items-center justify-center rounded-[28px] py-10 transition hover:bg-stone-50/70">
       <h3 className="text-center text-7xl font-black leading-tight text-red-500 md:text-8xl">{activeEntry.hanzi}</h3>
       <p className="mt-8 text-center text-xl font-black text-stone-500">Tự đoán pinyin và nghĩa trước khi lật thẻ.</p>
       <p className="mt-3 text-sm font-black uppercase tracking-wide text-stone-400">Space · Bấm thẻ để xem đáp án</p>
      </button>
     ) : (
      <FlashcardBack entry={activeEntry} onSpeak={onSpeak} onEdit={onEdit} compact />
     )}
    </div>
   </div>
   <div className="mt-7 flex flex-wrap justify-center gap-4">
    <RoundStudyButton icon={Search} label="Chi tiết" onClick={onEdit} />
    <RoundStudyButton icon={RotateCcw} label="Lùi" onClick={onReview} />
    <RoundStudyButton icon={revealed ? Eye : Play} label={revealed ? "Ẩn đáp án" : "Xem đáp án"} onClick={onReveal} primary />
    <RoundStudyButton icon={XCircle} label="Chưa nhớ" onClick={onAgain} danger />
    <RoundStudyButton icon={HelpCircle} label="Cần ôn" onClick={onReview} warning />
    <RoundStudyButton icon={Check} label="Đã nhớ" onClick={onRemember} success />
   </div>
  </div>
 );
}
...
function FlashcardBack({ entry, onSpeak, onEdit, compact }: { entry: VocabEntryWithProgress; onSpeak: () => void; onEdit: () => void; compact?: boolean }) {
 const analysis = entry.ai_analysis;
 const example = getPrimaryExample(entry);
 if (compact) {
  return (
   <div className="flex flex-1 flex-col overflow-y-auto px-2 py-5 text-center">
    <h3 className="text-6xl font-black text-red-500">{entry.hanzi}</h3>
    <p className="mt-5 text-3xl font-bold text-stone-900">{entry.pinyin}{(entry.sino_vietnamese || analysis.han_viet) ? ` - ${(entry.sino_vietnamese || analysis.han_viet || "").toUpperCase()}` : ""}</p>
    <p className="mx-auto mt-6 max-w-2xl text-2xl font-bold leading-10 text-stone-800">{getMeaning(entry)}</p>
    {example && (
     <div className="mt-7 border-t-2 border-stone-100 pt-6 text-left">
      <p className="text-xl font-black leading-9 text-stone-900"><Volume2 className="mr-2 inline h-6 w-6" />{example.zh}</p>
      <p className="mt-2 text-lg font-bold text-stone-500">{example.pinyin}</p>
      <p className="mt-3 text-xl font-bold leading-8 text-stone-800">{example.vi}</p>
     </div>
    )}
    <div className="mt-5 flex flex-wrap gap-2 text-left">
     {analysis.collocations?.slice(0, 4).map((item) => <span key={item} className="rounded-full bg-stone-100 px-3 py-1 text-sm font-black text-stone-700">{item}</span>)}
    </div>
   </div>
  );
 }
   ...
   <div className="rounded-[28px] border-2 border-stone-200 bg-white p-5 shadow-theme-sm">
    <div className="flex items-start justify-between gap-3">
     <div>
      ...
      {(entry.sino_vietnamese || analysis.han_viet) && <p className="mt-1 text-sm font-black text-stone-500">Hán Việt: {entry.sino_vietnamese || analysis.han_viet}</p>}
     </div>
    ...
    {entry.word_type && <span className="mt-4 inline-flex rounded-full bg-stone-100 px-3 py-1 text-xs font-black text-stone-600">{entry.word_type}</span>}
    <p className="mt-4 text-lg font-black leading-8 text-stone-800">{getMeaning(entry)}</p>
    {analysis.decomposition && (
     <div className="mt-4 rounded-2xl bg-yellow-50 p-3">
      <p className="text-xs font-black uppercase text-orange-600">Chiết tự</p>
      <p className="mt-1 text-sm font-bold leading-6 text-stone-700">{analysis.decomposition}</p>
     </div>
    )}
   </div>
   ...
   <div className="rounded-[28px] border-2 border-stone-200 bg-white p-5 shadow-theme-sm">
     ...
     <div>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
       {analysis.collocations.slice(0, 4).map((collocation) => (
        <span key={collocation} className="rounded-full bg-stone-100 px-3 py-1 text-sm font-black text-stone-700">{collocation}</span>
       ))}
      </div>
     </div>
    )}
    {example && (
     <div className="mt-5 rounded-2xl bg-stone-50 p-4">
      <p className="text-sm font-black leading-6 text-stone-900">{example.zh}</p>
      <p className="mt-1 text-xs font-bold text-red-500">{example.pinyin}</p>
      <p className="mt-2 text-sm font-bold leading-6 text-stone-600">{example.vi}</p>
      {example.note && <p className="mt-2 text-xs font-bold leading-5 text-stone-500">→ {example.note}</p>}
     </div>
    )}
    {(analysis.usage_note || analysis.cultural_note) && (
     <div className="mt-5 grid gap-2">
      {analysis.usage_note && <p className="rounded-2xl bg-blue-50 p-3 text-sm font-bold leading-6 text-blue-700">Lưu ý: {analysis.usage_note}</p>}
      {analysis.cultural_note && <p className="rounded-2xl bg-emerald-50 p-3 text-sm font-bold leading-6 text-emerald-700">Trung Việt: {analysis.cultural_note}</p>}
     </div>
    )}
    <Link
 ...
 return (
  <div className="mx-auto w-full max-w-4xl">
   ...
   <div className="mx-auto mt-6 rounded-[28px] border-2 border-stone-200 bg-white p-6 shadow-theme-sm md:p-8">
    <IconToolButton icon={Volume2} label="Phát âm" onClick={onSpeak} />
    <p className="mt-4 text-sm font-black uppercase tracking-wide text-stone-500">Định nghĩa</p>
    <p className="mx-auto mt-2 max-w-2xl text-2xl font-black leading-9 text-red-500">{getMeaning(activeEntry)}</p>
    {hiddenExample && (
     <div className="mx-auto mt-5 max-w-2xl rounded-2xl bg-stone-50 p-4 text-left">
      <p className="text-xs font-black uppercase tracking-wide text-stone-500">Ví dụ</p>
      <p className="mt-2 text-base font-black leading-7 text-stone-800">{hiddenExample}</p>
    ...
    {hintLevel > 0 && (
     <div className="mx-auto mt-5 max-w-lg rounded-2xl border-2 border-dashed border-yellow-300 bg-yellow-50 p-4 text-left">
      ...
      <ul className="mt-2 space-y-1 text-sm font-bold text-stone-700">
       {hints.slice(0, hintLevel).map((hint) => <li key={hint}>• {hint}</li>)}
      </ul>
     </div>
    )}
    {guessState !== "idle" && (
     <div className={cn("mx-auto mt-5 max-w-lg rounded-2xl border-2 p-4", guessState === "correct" ? "border-emerald-300 bg-emerald-50" : "border-red-300 bg-red-50")}>
      <p className="text-sm font-black text-stone-800">Đáp án: {activeEntry.hanzi} · {activeEntry.pinyin}</p>
      <p className="mt-1 text-sm font-bold text-stone-600">{getMeaning(activeEntry)}</p>
     </div>
    )}
    <form
    ...
    >
     ...
     {guessState === "idle" ? (
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
       <ActionButton onClick={onAgain} tone="neutral">Không biết</ActionButton>
       <ActionButton onClick={onHint} tone="neutral">H · Gợi ý</ActionButton>
       <ActionButton onClick={onCheckGuess}>Kiểm tra</ActionButton>
      </div>
     ) : (
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
       <ActionButton onClick={onAgain} tone="neutral">Chưa nhớ</ActionButton>
       <ActionButton onClick={onReview} tone="neutral">Cần ôn</ActionButton>
...
function QuizMode({
 ...
 return (
  <div className="mx-auto w-full max-w-4xl">
   ...
   <div className="mx-auto mt-6 rounded-[28px] border-2 border-stone-200 bg-white p-6 shadow-theme-sm md:p-8">
    <IconToolButton icon={Volume2} label="Phát âm" onClick={onSpeak} />
    <h3 className={cn("mx-auto mt-4 max-w-2xl font-black leading-tight", reverse ? "text-6xl text-red-500" : "text-3xl text-blue-500")}>
     {reverse ? activeEntry.hanzi : getMeaning(activeEntry)}
    </h3>
    <p className="mt-4 text-lg font-bold text-stone-600">{reverse ? activeEntry.pinyin : "Chọn Hán tự đúng"}</p>
      ...
      return (
       <button
        ...
        className={cn(
         ...
         answerChecked && isSelected && !isCorrect && "border-red-400 bg-red-50",
         !answerChecked && "border-stone-200 hover:border-red-300 hover:bg-red-50/30",
        )}
       >
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-stone-300 text-lg font-black text-stone-600">{answerLabels[index]}</span>
        <span>
         <span className="block text-2xl font-black text-red-500">{reverse ? getMeaning(option) : option.hanzi}</span>
         <span className="mt-1 block text-sm font-bold leading-6 text-stone-500">
          {reverse ? (answerChecked ? option.hanzi : "Chọn nghĩa phù hợp") : option.pinyin}
         </span>
         {!reverse && answerChecked && <span className="mt-1 block text-sm font-bold text-stone-500">{getMeaning(option)}</span>}
        </span>
       </button>
      );
     })}
    ...
    {answerChecked && (
     <div className={cn("mx-auto mt-5 max-w-3xl rounded-2xl border-2 p-4 text-left", correct ? "border-emerald-300 bg-emerald-50" : "border-red-300 bg-red-50")}>
      <p className="text-sm font-black text-stone-900">{correct ? "Đúng rồi" : `Chưa đúng. Đáp án: ${activeEntry.hanzi}`}</p>
      <p className="mt-1 text-sm font-bold leading-6 text-stone-600">{getMeaning(activeEntry)}</p>
      {selected && !correct && <p className="mt-1 text-xs font-bold text-stone-500">Bạn chọn: {selected.hanzi} · {getMeaning(selected)}</p>}
      <div className="mt-4">
       <ActionButton onClick={onContinueQuiz}>{correct ? "Tiếp tục" : "Ghi vào cần ôn"}</ActionButton>
      </div>
     </div>
    )}
   </div>
...
function WriteMode({
 ...
 const activeChar = characters[writeCharIndex] || characters[0] || activeEntry.hanzi.slice(0, 1);
 return (
  <div className="mx-auto w-full max-w-4xl">
   <StudyMeta entry={activeEntry} cardIndex={cardIndex} total={total} />
   <div className="mx-auto mt-6 rounded-[28px] border-2 border-stone-200 bg-white p-6 shadow-theme-sm md:p-8">
    <h3 className="text-5xl font-black text-red-500 md:text-6xl">{activeEntry.hanzi}</h3>
    ...
    <p className="mx-auto mt-3 max-w-2xl text-lg font-bold leading-8 text-stone-600">{getMeaning(activeEntry)}</p>
    <div className="mt-7 flex flex-wrap justify-center gap-2">
     {characters.map((character, index) => (
      <button
       ...
       className={cn("flex h-12 w-12 items-center justify-center rounded-xl border-2 text-xl font-black shadow-theme-sm", index === writeCharIndex ? "border-red-500 bg-red-500 text-white" : "border-stone-200 bg-white text-stone-800 hover:bg-stone-50")}
      >
       {character}
      </button>
     ))}
...
function StatBox({ value, label, tone }: { value: number; label: string; tone: "yellow" | "blue" | "green" }) {
 ...
 return (
  <div className={cn("min-w-24 rounded-2xl border-2 px-4 py-3 text-center shadow-theme-sm", className)}>
 ...
 return (
  <section className="rounded-[28px] border-2 border-stone-200 bg-white p-5 shadow-theme-md md:p-7">
   <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
    <div>
     <p className="text-sm font-black uppercase tracking-wide text-red-500">Tổng hợp</p>
     <h2 className="text-3xl font-black text-stone-900">Tất cả từ vựng</h2>
     <p className="mt-2 text-sm font-bold text-stone-500">{entries.length} thẻ đang hiển thị · {lessons.length} bài</p>
    </div>
    <div className="flex w-full flex-col gap-3 sm:flex-row lg:max-w-2xl">
     <div className="relative min-w-0 flex-1">
      <Search className="absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-stone-400" />
      <Input value={searchQuery} onChange={(event) => onSearchChange(event.target.value)} placeholder="Tìm Hán tự, pinyin, nghĩa..." className="h-12 rounded-2xl border-2 border-stone-200 bg-white pl-12 text-base font-bold" />
     </div>
     <ActionButton onClick={onAddEntry} icon={Plus}>Thêm từ</ActionButton>
    </div>
   </div>
   <FilterBar value={wordFilter} onChange={onFilterChange} />
   <div className="mt-5 grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
    {entries.map((entry) => <WordCard key={entry.id} entry={entry} onEdit={() => onEdit(entry)} />)}
   </div>
  </section>
 );
}

function FilterBar({ value, onChange }: { value: WordFilter; onChange: (filter: WordFilter) => void }) {
  ...
  <div className="mt-5 flex flex-wrap gap-2">
   {filters.map((filter) => (
    <button
     ...
     className={cn("h-9 rounded-xl border-2 px-3 text-xs font-black transition", value === filter.key ? "border-red-500 bg-red-500 text-white" : "border-stone-200 bg-white text-stone-600 hover:bg-stone-50")}
    >
     {filter.label}
    </button>
   ))}
...
function WordCard({ entry, onEdit }: { entry: VocabEntryWithProgress; onEdit: () => void }) {
 return (
  <article className="rounded-[24px] border-2 border-stone-200 bg-white p-4 shadow-theme-sm">
   <div className="flex items-start justify-between gap-3">
    <Link href={`/dictionary/${encodeURIComponent(entry.hanzi)}`} className="min-w-0">
     <h3 className="truncate text-3xl font-black text-stone-900">{entry.hanzi}</h3>
     <p className="mt-1 truncate text-sm font-black text-red-500">{entry.pinyin}</p>
    </Link>
    <StatusPill status={entry.status} />
   </div>
   <p className="mt-3 line-clamp-2 min-h-11 text-sm font-bold leading-6 text-stone-600">{getMeaning(entry)}</p>
   <div className="mt-4 flex items-center justify-between border-t-2 border-stone-100 pt-3">
    <span className="truncate text-xs font-black uppercase tracking-wide text-stone-400">{entry.source.lessonKey} · {entry.category || "Bổ sung"}</span>
    <button type="button" onClick={onEdit} className="inline-flex h-9 items-center gap-2 rounded-xl px-3 text-sm font-black text-stone-600 hover:bg-stone-100">
     <PenLine className="h-4 w-4" />
     Sửa
    </button>
   </div>
 ...
 return (
  <section className="rounded-[28px] border-2 border-stone-200 bg-white p-5 shadow-theme-md md:p-7">
   <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
    <div>
     <p className="text-sm font-black uppercase tracking-wide text-red-500">Quản lý bài</p>
     ...
     <article key={lesson.id} className="rounded-[24px] border-2 border-stone-200 bg-white p-4 shadow-theme-sm">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
       <div>
        ...
        <p className="mt-1 text-sm font-bold text-stone-500">{lesson.lesson_key} · {lesson.entries.length} từ · order {lesson.lesson_order}</p>
       </div>
       <div className="flex flex-wrap gap-2">
        <ActionButton onClick={() => onAddEntry(lesson)} tone="neutral" icon={Plus}>Thêm từ</ActionButton>
        <ActionButton onClick={() => onImportLesson(lesson)} tone="neutral" icon={Upload}>Import bài</ActionButton>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
       {lesson.entries.slice(0, 12).map((entry) => (
        <button key={entry.id} type="button" onClick={() => onEditEntry(entry)} className="rounded-2xl border-2 border-stone-200 bg-stone-50 p-3 text-left hover:bg-white">
         <span className="block text-lg font-black text-stone-900">{entry.hanzi}</span>
         <span className="block truncate text-xs font-bold text-stone-500">{getMeaning(entry)}</span>
        </button>
       ))}
      </div>
     </article>
...
function ImportLessonDrawer({
 ...
 const [manual, setManual] = useState<ImportedEntryDraft>(() => createManualEntryDraft(lesson));
 const parsedEntries = useMemo(() => parseMarkdownVocabEntries(pasteText, lesson), [lesson, pasteText]);
 const manualReady = Boolean(manual.hanzi.trim() && manual.meaning.trim());

 const updateManual = (patch: Partial<ImportedEntryDraft>, analysisPatch?: Partial<AiAnalysis>) => {
  setManual((current) => ({
   ...current,
   ...patch,
   ai_analysis: {
    ...
    ...(patch.sino_vietnamese !== undefined ? { sino_vietnamese: patch.sino_vietnamese, han_viet: patch.sino_vietnamese } : {}),
    ...
    <div className="grid grid-cols-2 rounded-2xl bg-stone-100 p-1">
     {[
      ...
      <button
       ...
       className={cn("h-10 rounded-xl text-sm font-black transition", mode === item.key ? "bg-white text-red-500 shadow-theme-sm" : "text-stone-500 hover:text-stone-900")}
      >
       {item.label}
      </button>
     ))}
     ...
     <>
      <Field label="Dán block markdown từ AI">
       <Textarea
        ...
        rows={14}
        placeholder={"## 算命 – suànmìng – Toán mệnh – Bói toán, xem bói\n*Động từ 【动】*\n\n**1. Hán Việt & Liên hệ Tiếng Việt**\n..."}
       />
      </Field>
      <div className="rounded-2xl border-2 border-stone-200 bg-stone-50 p-4">
       <p className="text-sm font-black text-stone-900">Preview: {parsedEntries.length} từ hợp lệ</p>
       <div className="mt-3 max-h-44 overflow-y-auto space-y-2">
        {parsedEntries.slice(0, 8).map((entry) => (
         <div key={`${entry.hanzi}-${entry.row_number}`} className="rounded-xl bg-white px-3 py-2 text-sm font-bold text-stone-700">
          <span className="font-black text-red-500">{entry.hanzi}</span> · {entry.pinyin} · {entry.meaning}
         </div>
        ))}
       </div>
      </div>
      <div className="sticky bottom-0 flex flex-wrap gap-3 border-t-2 border-stone-100 bg-white py-4">
       ...
       <ActionButton onClick={() => onImport(parsedEntries)} icon={Upload}>Import {parsedEntries.length} từ</ActionButton>
      </div>
     </>
    ) : (
     <>
      <div className="grid gap-4 md:grid-cols-2">
       <Field label="Hán tự"><Input value={manual.hanzi} onChange={(event) => updateManual({ hanzi: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
       <Field label="Pinyin"><Input value={manual.pinyin} onChange={(event) => updateManual({ pinyin: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
       <Field label="Hán Việt"><Input value={manual.sino_vietnamese || ""} onChange={(event) => updateManual({ sino_vietnamese: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
       <Field label="Loại từ"><Input value={manual.word_type || ""} onChange={(event) => updateManual({ word_type: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
       <Field label="Category"><Input value={manual.category || ""} onChange={(event) => updateManual({ category: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
       <Field label="Row"><Input type="number" value={manual.row_number || ""} onChange={(event) => updateManual({ row_number: Number(event.target.value) })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
      </div>
      <Field label="Nghĩa ngắn"><Input value={manual.meaning} onChange={(event) => updateManual({ meaning: event.target.value }, { meaning_detail: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
      <Field label="Chiết tự"><Textarea value={manual.ai_analysis.decomposition || ""} onChange={(value) => updateManual({}, { decomposition: value })} /></Field>
      <Field label="So sánh gần nghĩa"><Textarea value={lineText(manual.ai_analysis.comparisons)} onChange={(value) => updateManual({}, { comparisons: parseLines(value) })} /></Field>
      <Field label="Cụm từ cố định"><Textarea value={lineText(manual.ai_analysis.collocations)} onChange={(value) => updateManual({}, { collocations: parseLines(value) })} /></Field>
      <Field label="Ví dụ: zh | pinyin | vi | note"><Textarea value={examplesToText(manual.ai_analysis.examples)} onChange={(value) => updateManual({}, { examples: textToExamples(value) })} rows={6} /></Field>
      <Field label="Văn hoá / Trung Việt"><Textarea value={manual.ai_analysis.cultural_note || ""} onChange={(value) => updateManual({}, { cultural_note: value })} /></Field>
      <Field label="Lưu ý"><Textarea value={manual.ai_analysis.usage_note || ""} onChange={(value) => updateManual({}, { usage_note: value })} /></Field>
      <div className="sticky bottom-0 flex flex-wrap gap-3 border-t-2 border-stone-100 bg-white py-4">
       <ActionButton onClick={onClose} tone="neutral">Huỷ</ActionButton>
       <ActionButton onClick={() => onImport([manual])} icon={Plus} disabled={!manualReady}>Thêm từ</ActionButton>
      </div>
     </>
    )}
   </div>
...
function createManualEntryDraft(lesson: VocabLessonWithStats): ImportedEntryDraft {
 const category = lesson.categories[0]?.name || "Bổ sung";
 const rowNumber = lesson.entries.length + 1;
 return {
  hanzi: "",
...
function EntryEditDrawer({
 ...
 const [examplesText, setExamplesText] = useState(examplesToText(entry.ai_analysis.examples));
 const [collocationsText, setCollocationsText] = useState(lineText(entry.ai_analysis.collocations));
 const [comparisonsText, setComparisonsText] = useState(lineText(entry.ai_analysis.comparisons));

 const updateAnalysis = (patch: Partial<AiAnalysis>) => setAnalysis((current) => ({ ...current, ...patch }));
 const save = () => {
 ...
 return (
  <Drawer title={entry.id.startsWith("new-") ? "Thêm từ mới" : `Sửa từ ${entry.hanzi}`} onClose={onClose}>
   <div className="grid gap-4">
     ...
     <select
      ...
      onChange={(event) => {
       ...
       setDraft({
        ...
        row_number: nextLesson ? nextLesson.entries.length + 1 : draft.row_number,
        source: nextLesson
         ? {
            ...draft.source,
    ...
    <Field label="Row trong bài"><Input type="number" value={draft.row_number} onChange={(event) => setDraft({ ...draft, row_number: Number(event.target.value) })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
    <Field label="Hán tự"><Input value={draft.hanzi} onChange={(event) => setDraft({ ...draft, hanzi: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 bg-white font-bold" /></Field>
    ...
    <Field label="Hán Việt"><Input value={draft.sino_vietnamese || ""} onChange={(event) => setDraft({ ...draft, sino_vietnamese: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
    <Field label="Nghĩa ngắn"><Input value={draft.meaning} onChange={(event) => setDraft({ ...draft, meaning: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
    <Field label="Loại từ"><Input value={draft.word_type || ""} onChange={(event) => setDraft({ ...draft, word_type: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
    <Field label="Category"><Input value={draft.category || ""} onChange={(event) => setDraft({ ...draft, category: event.target.value })} className="h-11 rounded-2xl border-2 border-stone-200 font-bold" /></Field>
    <Field label="Nghĩa chi tiết"><Textarea value={analysis.meaning_detail || ""} onChange={(value) => updateAnalysis({ meaning_detail: value })} /></Field>
    <Field label="Chiết tự"><Textarea value={analysis.decomposition || ""} onChange={(value) => updateAnalysis({ decomposition: value })} /></Field>
    <Field label="So sánh gần nghĩa"><Textarea value={comparisonsText} onChange={setComparisonsText} /></Field>
    <Field label="Cụm từ cố định"><Textarea value={collocationsText} onChange={setCollocationsText} /></Field>
    <Field label="Ví dụ: zh | pinyin | vi | note"><Textarea value={examplesText} onChange={setExamplesText} rows={7} /></Field>
    <Field label="Văn hoá / Trung Việt"><Textarea value={analysis.cultural_note || ""} onChange={(value) => updateAnalysis({ cultural_note: value })} /></Field>
    <Field label="Lưu ý"><Textarea value={analysis.usage_note || ""} onChange={(value) => updateAnalysis({ usage_note: value })} /></Field>
    <div className="sticky bottom-0 flex flex-wrap gap-3 border-t-2 border-stone-100 bg-white py-4">
     {!entry.id.startsWith("new-") && <ActionButton onClick={() => onDelete(entry)} tone="neutral" icon={Trash2}>Xoá từ</ActionButton>}
     <ActionButton onClick={onClose} tone="neutral">Huỷ</ActionButton>
...
function LessonEditDrawer({
 ...
 return (
  <Drawer title={lesson.id.startsWith("new-") ? "Thêm bài mới" : "Sửa bài"} onClose={onClose}>
   <div className="grid gap-4">
    ...
    <div className="rounded-2xl bg-stone-50 p-4">
     <p className="text-sm font-black text-stone-700">{lesson.entries.length} từ trong bài</p>
     <p className="mt-1 text-xs font-bold text-stone-500">Có thể chuyển từng từ sang bài khác trong drawer sửa từ. Dùng row number để giữ thứ tự học.</p>
    </div>
    <div className="sticky bottom-0 flex flex-wrap gap-3 border-t-2 border-stone-100 bg-white py-4">
  ...
  <div className="fixed inset-0 z-50 flex justify-end">
   ...
   <aside className="relative h-full w-full max-w-xl overflow-y-auto border-l-2 border-stone-200 bg-white p-5 shadow-2xl">
    <div className="mb-5 flex items-center justify-between gap-3">
     ...
     <button type="button" onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-2xl border-2 border-stone-200 text-stone-600 hover:bg-stone-50">
      <X className="h-5 w-5" />
     </button>
    </div>
    {children}
  ...
  <div className="fixed inset-0 z-50 lg:hidden">
   ...
   <div className="absolute inset-x-0 bottom-0 max-h-[82vh] overflow-y-auto rounded-t-[28px] border-2 border-stone-200 bg-white p-4 shadow-2xl">
    ...
    <div className="grid gap-3">
     {lessons.map((lesson) => (
      <LessonCard key={lesson.id} lesson={lesson} active={lesson.id === activeLessonId} onClick={() => onSelect(lesson)} />
     ))}
    </div>
   </div>
  </div>
...
function EmptyState({
 ...
 return (
  <div className={cn("flex flex-col items-center justify-center rounded-[28px] border-2 border-stone-200 bg-white p-8 text-center shadow-theme-md", compact ? "min-h-64" : "min-h-[520px]")}>
