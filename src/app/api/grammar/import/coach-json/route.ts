...
import { isMissingGrammarTableError, parseHanyuGrammarCoachJson } from "@/services/grammar-learning.service";
import type { DbGrammarCourse } from "@/types/database";

export const dynamic = "force-dynamic";

const DEFAULT_JSON_PATH = "/Users/hagenlee/Desktop/USSH/HK2-USSH/han_ngu_grammar_11_25_data.json";
...
export async function POST(request: NextRequest) {
 ...
 try {
  const body = (await request.json().catch(() => ({}))) as { data?: unknown; preview?: boolean; reset?: boolean };
  const rawData = body.data || JSON.parse(await readFile(DEFAULT_JSON_PATH, "utf8"));
  const parsed = parseHanyuGrammarCoachJson(rawData as Parameters<typeof parseHanyuGrammarCoachJson>[0], "preview-course");
  if (!parsed.length) return NextResponse.json({ error: "Không parse được grammar coach JSON" }, { status: 400 });
  ...
  if (body.preview) {
   return NextResponse.json({
    lessons: parsed.map((lesson) => ({
     ...
     exercises: lesson.points.reduce((sum, point) => sum + point.exerciseDrafts.length, 0),
    })),
    totalLessons: parsed.length,
    totalPoints: parsed.reduce((sum, lesson) => sum + lesson.points.length, 0),
    totalExercises: parsed.reduce((sum, lesson) => sum + lesson.points.reduce((inner, point) => inner + point.exerciseDrafts.length, 0), 0),
   });
  }
  ...
  if (!course) {
   const { data, error } = await supabase
    ...
    .insert({
     ...
     title: "Hán ngữ 2 - Grammar Coach",
     source_type: "json",
     source_file: "han_ngu_grammar_11_25_data.json",
    })
    .select("*")
    .single();
  ...
  for (const lesson of parsed) {
   ...
   for (const point of lesson.points) {
    ...
    if (point.exerciseDrafts.length) {
     const exerciseRows = point.exerciseDrafts.map((exercise, index) => ({
      ...exercise,
      course_id: course.id,
      lesson_id: insertedLesson.id,
      point_id: insertedPoint.id,
     ...
     const { data: insertedExercises, error: exerciseError } = await supabase.from("grammar_exercises").insert(exerciseRows).select("id");
     if (exerciseError) return NextResponse.json({ error: exerciseError.message }, { status: 500 });
     importedExercises += insertedExercises?.length || 0;
    }
  ...
  return NextResponse.json({ importedLessons, importedPoints, importedExercises, courseId: course.id });
 } catch (error) {
  if (isMissingGrammarTableError(error as { code?: string; message?: string })) {
   return migrationRequired((error as Error).message);
  }
  return NextResponse.json({ error: error instanceof Error ? error.message : "Import grammar coach JSON thất bại" }, { status: 500 });
 }
}
