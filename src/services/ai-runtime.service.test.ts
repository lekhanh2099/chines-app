...
describe("shared AI runtime resolver", () => {
 it("keeps schema and vault failures distinct from a missing user key", () => {
  ...
  expect(
   getAiRuntimeReadinessFromInventory(inventory({ activeKeys: [], credentials: [] })),
 ...
 it("distinguishes transient provider rate limits from exhausted account quota", () => {
  ...
  expect(classifyAiRuntimeOperationFailure({ status: 429, message: "insufficient_quota" })).toBe(
   "quota-exhausted",
  );
