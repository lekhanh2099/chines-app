...
export function isMissingGrammarTableError(
...
export function buildGrammarPayload({
 ...
 const pointsWithProgress: GrammarPointWithProgress[] = points.map((point) => {
  ...
  return {
   ...
   exercises: (exercisesByPoint.get(point.id) || []).sort(
 ...
 const lessonPayload: GrammarLessonWithStats[] = lessons.map((lesson) => {
  const lessonPoints = (pointsByLesson.get(lesson.id) || []).sort(
...
function parseQuickExample(value: string): GrammarPointContent["quick_example"] {
 ...
 return {
  ...
  vi:
   lines.find((line) => !line.includes("(") && line !== first && line !== lines[1]) ||
   lines[2] ||
   "",
 };
}
...
function firstExampleForQuickExample(
 examples: GrammarPointContent["examples"],
 fallbackTitle: string,
): GrammarPointContent["quick_example"] {
 ...
 return blocks.map((lessonMatch) => {
  ...
  return {
   ...
   points: pointBlocks.map((pointMatch, index) => {
    ...
    const comparisons = pointBlock.includes("|")
     ? parseList(
        pointBlock
         .split("\n")
    ...
    return {
     title: pointTitle,
     hanzi: /[\u3400-\u9fff]/.test(pointTitle)
      ? pointTitle.match(/[\u3400-\u9fff][\u3400-\u9fff\s+/.·…、“”]+/)?.[0]?.trim()
      : undefined,
     pinyin: undefined,
...
export function parseHanyuGrammarCoachJson(
 data: CoachGrammarData,
 courseId: string,
): ParsedGrammarLesson[] {
  ...
  .map(([lessonNumber, lessonCards]) => {
   ...
   return {
    ...
    points: lessonCards.map((card, index) => {
     ...
     const quiz =
      card.quiz?.q && card.quiz?.choices?.length
       ? {
          q: card.quiz.q,
     ...
     return {
      ...
      hanzi: /[\u3400-\u9fff]/.test(card.point)
       ? card.point.match(/[\u3400-\u9fff][\u3400-\u9fff\s+/.·…、“”]+/)?.[0]?.trim()
       : undefined,
      pinyin: undefined,
 ...
 return parseList(value).map((line, index) => {
  ...
  const content: GrammarExerciseContent = {
   ...
   ...(type === "reorder_sentence"
    ? { tokens: line.replace(/^.*?:/, "").trim().split(/\s+/).filter(Boolean) }
    : {}),
...
export function parseGrammarMarkdown(
 value: string,
 lesson: DbGrammarLesson,
 courseId: string,
): ParsedGrammarPoint[] {
 const blocks = value
  .split(/\n---+\n|(?=\n##\s+)/g)
 ...
 return blocks.map((block, index) => {
  ...
  return {
   ...
   row_number: index + 1,
   content,
   exerciseDrafts: parseExerciseLines(
    section(block, "Bài tập|Luyện tập"),
    courseId,
    lesson.id,
    null,
   ),
...
function optionSet(answer: string, pool: string[], index: number) {
 const distractors = rotate(
  pool.filter((item) => normalizePlain(item) !== normalizePlain(answer)),
  index,
 ).slice(0, 3);
...
function getPointPrompt(point: GrammarPointWithProgress) {
 return (
  ...
  point.content.core ||
  point.content.explanation?.split(/[.\n]/)[0] ||
  point.title
 );
}
...
function getRequiredTerms(point: GrammarPointWithProgress) {
 const source = [
  ...
  .filter(Boolean)
  .join(" ");
 return Array.from(source.matchAll(/[\u3400-\u9fff]+/g))
  .map((match) => match[0])
  .slice(0, 4);
}
...
> {
 return {
  ...
  explanation:
   ...
   point.content.core ||
   point.content.usage_notes?.[0] ||
   `Ôn lại cấu trúc ${point.title}.`,
  exercise_order: index + 1,
 };
}
...
export function generateGrammarExercisesForPoint({
 ...
 const examples = point.content.examples?.length
  ? point.content.examples
  : [point.content.quick_example].filter(Boolean);
 const structures = point.content.formulas?.length
  ? point.content.formulas
  : point.content.structures || [];
 ...
 return Array.from({ length: Math.max(1, count) }).map((_, index) => {
  ...
  if (type === "multiple_choice") {
   ...
   const correct =
    choices.find((choice) => normalizePlain(choice.text) === normalizePlain(answer))?.id || "A";
   return {
    ...base,
  ...
  if (type === "translate_zh") {
   return {
    ...
    content: {
     ...(base.content || {}),
     sample_answer: example?.zh || answer,
  ...
  if (type === "identify_error") {
   return {
    ...
    content: {
     ...(base.content || {}),
     accepted_answers: [point.title, structure].filter(Boolean),
    },
