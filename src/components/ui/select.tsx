...
function SelectGroup({ className, ...props }: React.ComponentProps<typeof SelectPrimitive.Group>) {
 return (
  <SelectPrimitive.Group
   ...
   className={cn("scroll-p-1", className)}
   {...props}
  />
 );
}
...
function SelectItem({
 ...
 return (
  <SelectPrimitive.Item
   ...
   className={cn(
    "relative flex min-h-10 w-full cursor-default items-center gap-2 rounded-lg py-2 pr-8 pl-2.5 text-sm leading-snug outline-hidden select-none focus:bg-accent focus:text-foreground not-data-[variant=destructive]:focus:text-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
    className,
   )}
   {...props}
  >
...
function SelectSeparator({
 ...
 return (
  <SelectPrimitive.Separator
   ...
   className={cn("pointer-events-none h-px bg-border", className)}
   {...props}
  />
 );
}
