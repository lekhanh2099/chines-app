...
function collectContentNodes(document: unknown): ContentNode[] {
 ...
 function visit(current: unknown, currentPath: string, owner: NodePointer) {
  if (Array.isArray(current)) {
   current.forEach((item, itemIndex) => visit(item, joinJsonPath(currentPath, itemIndex), owner));
   return;
  }
...
function createGroupRelations(params: {
 ...
 params.groups.forEach((group, groupIndex) => {
  ...
  const words = arrayValue(group.words).filter((word): word is string => typeof word === "string");
...
async function materializeDataset(
 ...
 for (const lessonFileName of lessonFiles) {
  ...
  const contentToVocab = dedupeRelations([
   ...explicitResult.relations.filter((relation) => relation.type === "content_to_vocabulary_item"),
...
async function main() {
 ...
 const rootManifest = {
  ...
  counts: {
   ...
   checkNeededRelations: results.reduce((sum, item) => sum + item.counts.checkNeededRelations, 0),
   checkNeededVocabItems: results.reduce((sum, item) => sum + item.counts.checkNeededVocabItems, 0),
  },
 };
